...
window.dataTable = function () {
  return {
    ...
    initData() {
      ...
      const urllast = window.location.pathname;

      // Menggunakan split untuk memisahkan path berdasarkan '/'
      const segments = urllast.split("/");

      // Mengambil bagian terakhir dari path
      const lastSegment = segments.pop();

      // const url = selectedId ? `` : "/get-plants";

      fetch(`/get-plants-location/${lastSegment}`)
        .then((response) => response.json())
        .then((data) => {
          this.items = data.sort(this.compareOnKey("no", "asc"));
