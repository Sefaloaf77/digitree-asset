    ...
    <div class="w-full px-6 py-6 mx-auto h-screen max-h-full bg-white">
        ...
        <div class="flex flex-wrap my-6 -mx-3">
            ...
            <div class="w-full">
                ...
                @if (session('success'))
                    <div
                        class="my-4 rounded w-full p-3 bg-green-digitree/10 text-green-digitree border border-green-digitree/60">
                        {{ session('success') }}
                    </div>
                @endif
                @if (session('error'))
                    <div class="my-4 rounded w-full p-3 bg-red-500/10 text-red-500 border border-red-500/60">
                        {{ session('error') }}
                    </div>
                @endif
            </div>
            ...
            @if ($errors->any())
                <div class=" rounded w-full p-3 bg-red-500/10 text-red-500 border border-red-500/60 mb-4">
                    <ul>
            ...
            <div class="w-full max-w-full px-3 mt-0 mb-6 md:mb-0 md:flex-none lg:flex-none">
                ...
                <div
                    ...
                    <div class="overflow-x-auto p-6 px-5 pb-2">
                        <form action="{{ route('dashboard.user-role.update', ['id' => $user->id]) }}" method="POST"
                            enctype="multipart/form-data">
                            @method('put')
                            @csrf
                            <div class="grid md:grid-cols-2 grid-cols-1 md:gap-10 mb-5">
                                <div class="w-full space-y-4">
                                    <label class="text-base font-semibold mb-4" for="name">Nama</label>
                            ...
                            <div class="mb-5 w-full">
                                @php
                                    $selectedVillages = $userVillage->pluck('id_village')->toArray(); // Extract selected village IDs
                                @endphp
                                <label class="text-base font-semibold mb-4 capitalize">Akses</label>
                                <div class="grid md:grid-cols-4 mt-5 gap-8">
                                    @foreach ($villages as $item)
                                        <div class="flex items-center gap-3">
                                            <input type="checkbox" name="id_village[]" value="{{ $item->id }}"
                                                {{ in_array($item->id, $selectedVillages) ? 'checked' : '' }}>
                                            <label for="id_village[]">{{ $item->name }} - {{ $item->kab_kota }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
